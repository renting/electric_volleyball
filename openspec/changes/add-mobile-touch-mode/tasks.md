...
## 3. 觸控操作面板（HTML / CSS overlay）

- [x] 3.1 在 `#wrap` 內新增觸控面板 DOM：左下方向群組（`←`、`→`、`↑`）與右下動作群組（扣殺/撲球 `hit`），預設以 class 隱藏。
- [x] 3.2 撰寫面板 CSS：絕對定位、半透明圓鈕、`touch-action:none`、`user-select:none`，並以 `env(safe-area-inset-*)` 避開瀏海/圓角。
- [x] 3.3 為每個按鈕綁定 `touchstart`/`pointerdown` → 設對應 `touchInput.*=true`；`touchend`/`touchcancel`/`pointerup`/`pointerleave` → 設 `false`；於 `touchstart` 呼叫 `preventDefault()`。
- [x] 3.4 僅在 `isTouchDevice()` 為真時移除隱藏 class 顯示面板；桌面維持不顯示。
- [ ] 3.5 驗證多點觸控：同時按住方向鈕 + 扣殺鈕可觸發移動中扣殺與「方向 + 扣殺」撲球；左右同時按住視為無方向、不撲球。（需觸控裝置實測）
- [ ] 3.6 驗證放開與滑出邊界（`touchcancel`/`pointerleave`）不會卡鍵。（需觸控裝置實測）
