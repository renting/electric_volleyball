...
- **手感調校**：死區（deadzone）需取足夠大（實作約為底座半徑的一半），避免輕碰即移動造成「太靈敏、不好操控」；門檻為實作可調參數，不影響移動速度（仍為固定 5.4）。

### 決策 7：視覺強化——球放大與扣殺彗星殘影
球半徑 17→28（同步套用於碰撞/網子/落地/繪製，維持物理一致；放大同時降低接球難度）；以 `ball.trail`（每物理幀記錄、上限約 16 筆）繪製漸層淡出殘影，`_tryPowerHit` 觸發 `ball.smash` 計時器使扣殺殘影最明顯並呈黃→橘彗星漸層，高速飛行時顯示較淡拖尾、低速不顯示。
